/*
  ...
  аппаратный последовательный порт настраивается с учетом скорости передачи данных, 
  битов данных, стоп-битов, четности при передаче по USB
  ...
  этот эскиз в основном предназначен для демонстрации / тестирования
  производительности связи CDC, поскольку реальный usb-последовательный мост был бы неприемлем, 
  поскольку каждый байт передается в отдельном USB-пакете
*/
//=======================================================================
uint32_t baud = 9600;
uint8_t databits = 8;
